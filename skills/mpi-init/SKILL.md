...
BACKLOG and COMPLETED entries on `.agents/mpi-kanban/kanban.md`.
...
<HARD-GATE>
Only ask for confirmation when importing into an EXISTING `.agents/mpi-kanban/kanban.md`
(risk of overwriting user data) Ã¢â‚¬â€ show the parsed entry list, get approval, then write.
...
freeform to-do file into `.agents/mpi-kanban/`) Ã¢â‚¬â€ just do it.
</HARD-GATE>
...
- If the user gives no file at all but asks to "set up the kanban", create
  the empty board from the template and stop Ã¢â‚¬â€ there is nothing to import.

## Checklist

Lib pointers (read each only when its recipe is needed in the steps below):

- `<mpi-lib-root>/kanban-ops/_schema.md` Ã¢â‚¬â€ entry shape (read before
  building entries if you need a schema reminder)
- `<mpi-lib-root>/kanban-ops/find.md` Ã¢â‚¬â€ `ensureKanban`
...
5. **Write entries** Ã¢â‚¬â€ read `<mpi-lib-root>/kanban-ops/mutate.md` for `createEntry`. Call
...
6. **Confirm** with a clickable kanban link: `[kanban.md](.agents/mpi-kanban/kanban.md)`
...
| `IDEAS`, `IDEA`                         | `[Idea]`    |
| `PLANS`, `PLAN`                         | `[PLAN]`    |
| `TASKS`, `TODO`, `TODOS`, `TO-DO`       | `[feature]` (default Ã¢â‚¬â€ bump to `[bug]` if the line says `bug:` / `fix:`) |
...
- `[ ] text` or `[x] text`
- `- [ ] text` or `- [x] text`
- `* text` or `- text` (no checkbox Ã¢â‚¬â€ treat as not-done)
- A standalone non-empty line under a section that isn't itself a heading

`[x]` (lowercase or uppercase) Ã¢â€ â€™ entry goes to `COMPLETED`. Otherwise Ã¢â€ â€™ `BACKLOG`.
...
Do NOT prompt the user per-entry for priority Ã¢â‚¬â€ that defeats the bulk-import
